...
Task 12 整合修订时再次运行 `date --iso-8601=seconds`，取得
`2026-09-16T02:23:46+08:00`。概览与启动篇不再使用未记录的午夜时间；本次改过正文的图形、音频和电源篇，以及任务 8 复审时改过正文的桌面应用篇，也采用该实测时间作为
`lastmod`。新稿的原 `date` 和七篇旧稿保留的发布日期没有改变。
