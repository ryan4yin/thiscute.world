...
每种哈希算法，基本都会推出多种输出长度的版本，最常用的是 256 位，视场景你也可能需要使用更高级别的版本如 512/1024 位。
...
- Whirlpool 生成出 512 位哈希值。

- SM3 是中国国密密码杂凑算法标准，由国家密码管理局于 2010 年 12 月公布。它类似于 SHA-256（基于 Merkle-Damgård 结构）并产生 256 位哈希值。

- GOST（GOST R 34.11-94）哈希函数是俄罗斯的国家标准，它也生成 256 位哈希值。
...
- Skein 能够导出 128、160、224、256、384、512 和 1024 位哈希值。
- Grøstl 能够导出 224、256、384 和 512 位哈希值。
- JH 能够导出 224、256、384 和 512 位哈希值。
...
这些挖矿算法的目标是通过刺激小型矿工（家庭用户和小型矿场）来**最大限度地减少挖矿的集中化**，并限制挖矿行业中大玩家（他们有能力建造巨型挖矿设施和数据中心）的力量。
与少数大玩家相比，**大量小玩家意味着更好的去中心化**。

大型虚拟货币挖矿公司手中的主要武器被认为是 ASIC 矿机，因此，现代加密货币的设计通常包括使用抗 ASIC 哈希算法或权益证明（proof-of-stake）共识协议进行「工作量证明挖矿」。
