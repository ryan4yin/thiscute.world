...
而**加密哈希函数**，因为带了个**加密**前缀，它对函数有更高的要求。
一个好的加密哈希函数必须满足**抗碰撞**（collision-resistant）和**不可逆**（irreversible）这两个条件。
...
在哈希表这种场景下，我们通常侧重于选择计算足够快、哈希值分布足够均匀或者随机的哈希函数，比如前面提到的「求和，乘一个素数 31」这样的简单算法。
这样的算法不要求安全性，称为「**非加密哈希函数**」
