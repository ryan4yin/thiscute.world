...
但是完全地实现 ACID 对性能的损耗仍然是非常严重的，在关系数据库中，设计者通常会选择牺牲相对不重要的「隔离性」来获取更好的性能。
...
- **脏写 Dirty writes**：即事务 T1 跟事务 T2 同时在原数据的基础上更新同一个数据，导致结果不符合预期。
  ...
  - 最简单的解决方法：针对 `UPDATE table SET field = field - 1000 WHERE id = 1` 这样的数据更新逻辑，需要对被更新的行加一把「行写锁」，使其他需要写此数据的事务等待。
...
- 「串行化」的性能太差，也直接排除
- 「不可重复读」问题在很多场景下只会造成不必要的计算，对业务逻辑的正确性并无影响，所以是可以容忍的。
