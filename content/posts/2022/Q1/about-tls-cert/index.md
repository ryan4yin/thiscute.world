...
featuredImage: "https-secure.webp"
resources:
  - name: featured-image
...
在现实世界中，Eve 这个角色被称作「**证书认证机构**（Certification Authority, CA）」，全世
...
PEM 格式的数据通常以 `.pem` `.key` `.crt` `.cer` 等拓展名存储，直接 `cat` 一下是不是字符
...
释，**通配符证书只支持同级匹配，，也就是说 `*` 不能匹配域名中的 `.` 这个字符**，详细说明如
下:

1. `*.aliyun.com` 能用于 `xx.aliyun.com` 和 `abc.aliyun.com`，但不能用于 `aliyun.com`,
   `a.b.aliyun.com` 或 `bb.xx.aliyun.com`.。
2. 一个证书能支持多个域名配置，比如一个包含了 `aliyun.com`, `*.a.aliyun.com`,
   `*.b.aliyun.com` 三个域名的证书，可以同时用于 `aliyun.com`, `xx.a.aliyun.com`,
   `yy.b.aliyun.com`。

要想保护多个二三级子域，只能在生成 TLS 证书时，添加多个通配符域名。因此设计域名规则时，要
...
> 但是我们也看到 X25519(`Curve25519`) 正在越来越流行，因为美国政府有前科
> （[NSA 在 RSA 加密算法中安置后门是怎么一回事，有何影响？——知乎](https://www.zhihu.com/question/22343037)），NIST
> 标准被怀疑可能有后门，目前很多人都在推动使用 `Curve25519` 等社区方案取代掉 NIST 标准曲
> 线。限于篇幅，感兴趣请自行了解。
...
1. **公网受信任证书** 或者叫 **Web PKI 证书**: 即由权威证书认证机构签名的的证书。
   - 几乎所有终端都预装了这些权威证书认证机构（CA）的根证书，因此这类证书会被浏览器、小程序等第三方应用/服务商验证
     并信任。
...
因此 **TLS 证书最好是设置自动轮转，并确保证书有效期过短时能触发告警！纯人工维护或纯自动化都不可靠**！
...
而在支持「完美前向保密（Perfect Forward Secrecy）」的 TLS1.2 或 TLS1.3 协议中，经典 DH 协
...
日志显示，客户端支持的协议有 `h2` 和 `http/1.1`，而服务端从中选择了 `http/1.1` 协议，于是
...
其原理是这些代理工具会在启动时首先要求将它自己的私有 CA 证书添加到系统证书中，这样客户端
...
举个例子，假设站点 `https://example.com/` 的响应头中有
