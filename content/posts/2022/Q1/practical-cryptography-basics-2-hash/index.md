...
**哈希函数**（Hash Function），或者叫**散列函数**，是一种为任何输入数据中创建一个**数字指纹**（也叫数字摘
要）的方法，哈希函数把数据压缩（或者放大）成一个长度固定的字符串。
...
逆**（irreversible）这两个条件。抗碰撞是指攻击者很难或者几乎不可能找到两个具有相同哈希值的输入，而不可逆则是说攻击者很难或几乎不可能从算法层面通过哈希值逆向演算出原始数
据。
