...
- 显示器
  ...
  - 如果你有需要显卡加速的桌面虚拟机，但是又不想搞复杂的显卡直通，可以选择 `VirGL GPU(virtio-gl)` 类型（注意不是 `VirtIO-GPU(virtio)`，这个驱动没有显卡加速能力），它能以较小的性能损耗将虚拟机中的 3D/2D 运算 offload 到 host GPU，而且避免复杂的驱动配置。但是目前它仅支持 Linux 4.4+ 的 Guest 主机，并且要求 mesa (>=11.2) compiled with the option `gallium-drivers=virgl`（我感觉这功能目前还有点鸡肋）。
