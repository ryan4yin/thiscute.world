...
总的来说，PVE 没有 [vSphere Hypervisor][vSphere Hypervisor] 跟 [Windows Hyper-V][Windows Hyper-V] 那么成熟、完善、稳定，但是基于 QEMU/KVM 且能够免费使用，很适合 Linux/开源/虚拟化 爱好者折腾。
...
1. 用 `fdisk` 先删除分区，再重新创建分区，实现修改分区表扩容。然后还需要用 `resize2fs` 扩容文件系统。细节请自行网上搜索文档。
...
1. [ryan4yin/pulumi-proxmox](https://github.com/ryan4yin/pulumi-proxmox): 我维护的一个 proxmox 自动配置工具（很久没更新了...）
