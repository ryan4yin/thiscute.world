...
>需要有一定 Linux 使用经验与编程经验，才能玩转 NixOS && Nix Flake，因此不推荐任何 Linux 新手选用 NixOS.
...
- 文档混乱：首先 Nix Flake 目前仍然是实验性特性，介绍它本身的文档目前比较匮乏。 其次 Nix 社区绝大多数文档都只介绍了旧的 `nix-env`/`nix-channel`，想直接从 Nix Flake 开始学习的话，需要参考大量旧文档，从中提取出自己需要的内容。另外一些 Nix 当前的核心功能，官方文档都语焉不详（比如 `imports` 跟 Nixpkgs Module System），想搞明白基本只能看源码了...
