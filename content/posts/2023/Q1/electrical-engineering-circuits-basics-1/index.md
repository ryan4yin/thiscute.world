...
玩了一阵后 ESPHome 这种小白式玩法就满足不了我了，于是开始学习电路知识，淘宝买了一堆元器件，用树莓派跟其他单片机开始折腾遥控小车、简易机械臂、跑马灯等等，可以说是玩得很尽兴。
...
1. 不论 $V_{CE}$ 在 1v - 20V 如何变化，$I_{C}$ 的电流变化都很小。
2. $V_{BE}$ 需要一个启动电压，大约在 0.7V 左右，低于 0.7V 时$I_C$ 的电流一直非常小。
3. 在 $V_{BE}$ 超过 0.7V 后，任何此电压的小变化，都会导致 $I_{C}$ 的剧烈变化。
...
有一定电路基础后，就可以开始玩单片机了，单片机学习顺序大概是：
...
单片机玩够了后，就可以开始玩 Linux 嵌入式开发了，相关资料（大致按学习顺序排序）：
