...
>插入下，这个 Endpoint 的地址也很值得一说，能看到服务端 wg0.conf 的配置中，peer1 并未被设置任何 Endpoint，这实质是表示这个 peer1 的 Endpoint 是动态确认的，也就是说每次 peer1 发送数据到服务端 wg0 时，服务端通过认证加密技术认证了数据后，就会以数据包的来源 IP 地址作为 peer1 的 Endpoint，这样 peer1 就可以随意更换自己的 IP 地址，而 WireGuard 隧道仍然能正常工作（IP 频繁更换的一个典型场景就是手机的网络漫游与 WiFi 切换）。这使 WireGuard 具备了比较明显的无连接特性，也就是说 WireGuard 隧道不需要保持一个什么连接，切换网络也不需要重连，只要数据包能够到达服务端，就能够正常工作。
...
结合注释看完上面的路由策略，现在你应该理清楚 WireGuard 的路由规则了，它加了条优先级更高的路由策略，将所有流量都通过它的自定义路由表路由到 peer1 接口，并通过一条额外的路由使 peer1 接口发送给服务端的流量能正常出站。

这里额外解释下 peer1 接口发送给服务端的流量如何正常出站，它是基于 `32764` 这条路由策略，该策略前面注释说了是让所有非默认路由的流量都走 main 路由表，那么我们看看 main 路由表里有哪些非默认路由规则：
...
能看到只有一条，就是 `192.168.5.0/24 dev eth0` 这条，而服务端的 IP 地址 `192.168.5.198` 刚好就能匹配到这条规则，所以 peer1 接口发送给服务端的流量就能正常出站了。

>实际上我这里服务端 IP 跟客户端在同一个局域网网段，请求根本不会上升到网络层路由，直接链路层 ARP 解析了，这里只是为了演示。
...
- [WireGuard 基础教程：wg-quick 路由策略解读 - 米开朗基扬](https://icloudnative.io/posts/linux-routing-of-wireguard/): 对 WireGuard 客户端用到的多路由表与路由策略技术做了详细的介绍。
