...
- `efibootmgr -v`：查看 / 修改固件启动顺序（传统发行版）；NixOS:
  `nix run nixpkgs#efibootmgr -v`
- `bootctl status`：检查 systemd-boot 安装与 ESP 状态
- `bootctl list`：列出启动条目
- `ukify inspect /boot/EFI/Linux/nixos-xxx.efi`: 查看 efi 镜像中包含的信息（需要安装
  systemd-ukify 包）
...
4. [`pam_gnome_keyring.so`](https://github.com/GNOME/gnome-keyring) 参数 (用于会话管理) 这
   个模块与 `sudo` 的认证流程无关，主要用于用户登录时解锁密钥环。
...
# 验证模块存在与架构匹配
# 传统发行版:
ls /lib/security/pam_unix.so  # 或 /lib64/security/
...
具体的调试命令请参考 [3.4.3 故障排查](#343-故障排查) 章节。
...
详细的 PAM、PolicyKit 和密钥管理调试命令请参考
[3.5 安全组件集成与最佳实践](#35-安全组件集成与最佳实践) 中的
[3.5.3 故障排查](#353-故障排查) 章节。
...
安全相关的调试命令请参考 [3.4.3 故障排查](#343-故障排查) 章节，该章节提供了完整的
