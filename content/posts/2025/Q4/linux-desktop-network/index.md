...
桌面上的网络图标能告诉我们连接到了哪个网络，但应用访问一个网站，还需要地址、路由、名称解析，以及对端响应。我在 NixOS 配置历史里记录过一次恢复后的网络问题：物理链路的变化牵动了 networkd 的重配置，进一步影响 VPN/TUN 的路由规则。要读懂这个案例，先得分清谁负责哪一段。

本文沿着接口出现、链路可用、取得地址、选择路由，再到 DNS 与应用连接展开。这是理解依赖的顺序；实际访问域名时，应用通常先做名称解析，再连接返回的地址，DNS 查询自身也需要网络路径。前面的[系统基础篇](/posts/linux-desktop-system-foundations/)解释了设备与服务如何出现，这里从内核已经注册网络接口开始。
...
这使得硬件变化与配置匹配之间有了直接联系。我的提交 `4098b028`
只改了一个网络变量中的接口名；它能证明配置引用的接口标识曾经调整，却没有提供启动日志或网络恢复测试。如果换网卡后出现了新接口，而配置仍按旧名字匹配，应该先检查匹配关系，不能因为 networkd 服务还在运行就认定新接口已被接管。

NixOS 把这一层写成声明：`systemd.network.enable` 启用集成，`systemd.network.networks` 生成
`.network` 文件，`matchConfig` 和 `networkConfig`
...
这些是当时写入提交与注释的记录，没有附带完整的恢复前后路由快照，也没有可据此独立重放的日志。它们支持把注意力放在重配置与规则归属上，不能证明所有恢复后的断网都有同一个原因。
...
如果要验证这个解释，需要把一次正常使用中发生的恢复事件前后串起来：carrier 何时变化，networkd 是否重配置，地址与路由是否被撤下，VPN/TUN 的策略规则是否变化，程序最终选择了哪个出口。只观察最后一个「网页打不开」的结果，无法分辨这些环节。此次修订没有触发挂起或重启网络服务，也没有补做恢复成功的验证。
...
把这些差异带回前面的依赖关系，「只有一个应用不能联网」就有了具体的检查方向：它采用哪种解析入口、是否连接代理、选中了哪个地址族，所发流量又会匹配哪条策略规则。系统当前有网络连接，是这些检查的起点。
