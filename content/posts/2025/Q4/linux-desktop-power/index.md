...
前面的篇章沿着开机、登录和应用运行一路展开。离开桌面时，系统还要决定怎样保存或结束这些工作：挂起让进程暂停，关机让它们退出；休眠则把可恢复的内存状态写入持久存储。三者经过的一些组件相同，最终保留的状态却不同。

我在 NixOS 配置历史里记录过一次 S3 恢复后的网络问题。它提醒我们，恢复桌面不仅要让内存里的程序继续运行，还要重新建立程序与设备、外部网络之间的联系。这个案例的网络细节放在[网络篇](/posts/linux-desktop-network/)，这里解释它发生的电源状态边界。
...
因此，不宜把挂起概括成「除了内存，所有东西都断电」。模式、总线和唤醒需求不同，设备实际状态也不同。同样，屏幕重新亮起只提供了图形这一端的证据，还需要分别确认输入、声音、存储和网络能否继续工作。
...
这里保留配置职责，不给出创建 swap 或重建引导配置的通用脚本。是否能恢复，取决于镜像位置、initramfs 能力、存储解锁和设备驱动共同满足条件，不能由某个选项已经写入配置推断。

## 恢复内存之后，网络仍会变化

前面说的设备恢复，是把设备带回可工作的状态。用户空间还会继续处理随后出现的设备与链路事件；保存在内存里的进程，可能面对与挂起前不同的网络条件。
...
这份提交能证明当时记录了上述解释，并实施了两项配置改动；它没有附上完整的恢复前后路由快照或独立的成功验证。十秒是配置选择，不是测得的恢复耗时。此次修订也没有触发挂起来重放该问题。
...
由此判断关机卡住的位置，比套一份耗时表更有用：是某个服务仍在停止，还是已经进入最后的文件系统与存储清理？下一次启动是[重新建立系统状态](/posts/linux-desktop-boot/)，与挂起后继续运行原进程有不同的证据路径。
...
带着这几种证据回看[系列全景](/posts/linux-desktop-architecture/)，同一台桌面便有了完整的时间线：启动建立状态，应用使用设备与服务，挂起暂存执行，恢复重接外部条件，关机结束进程并清理资源。
