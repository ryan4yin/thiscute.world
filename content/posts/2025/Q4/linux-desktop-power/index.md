...
桌面从 S3 恢复后，不只内存里的程序要继续运行，设备和网络连接也要重新进入可用状态。[网络篇](/posts/linux-desktop-network/)有一个网卡短暂断链并触发 networkd 重配置的案例，这里只看它与电源状态转换的关系。
...
在那个案例中，S3 恢复时 carrier 发生变化，networkd 随后重配置接口，VPN/TUN 的策略规则和出口选择也受到影响。配置分别调整了外部规则的管理边界和短暂断链的处理时机。具体配置、完整链接与取舍见[网络篇的恢复案例](/posts/linux-desktop-network/#一次恢复事件怎样影响到应用)。
